...
window.onload = function() {
    ...
    $("#view_invoices").addClass("active");    
    $('button[id^="ShowInvoice"]').on('click', function (event) {
            let id = $(this).attr("data-Invoice-id");
            $('button[id^="Download_file"]').attr("data-Invoice-id",id);
            $('span[id^="status_id"]').html('');
            console.log('clicked');
            $.ajax({
                url: '/flux/invoice/show/'+id,
    ...
    $('button[id^="Download_file"]').on('click', function (event) {
        ...
        let id_flux = $(this).attr("data-Invoice-id");
